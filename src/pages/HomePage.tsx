...
import { Award, Layers, BookOpen, ShieldCheck, ArrowRight, Clock, Star } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto px-4 space-y-12 sm:space-y-16">
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between border-b border-white/10 pb-3">
            ...
            <Link to="/airports" className="text-xs text-cyan-400 hover:underline flex items-center gap-1">
              查看机场大全 →
            </Link>
          </div>
            ...
            {topAirports.map((airport) => (
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs text-tan/80 pt-2 border-t border-white/5">
                  ...
                  <Link to={`/airport/${airport.slug}`} className="text-cyan-400 font-semibold hover:underline">
                    详情 →
                  </Link>
                </div>
              </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between border-b border-white/10 pb-3">
            ...
            <Link to="/airports" className="text-xs text-cyan-400 hover:underline">
              完整列表 →
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              ...
              <Link to="/guides" className="text-xs text-cyan-400 hover:underline">更多 →</Link>
            </div>
            <div className="space-y-3">
              {guides.map(g => (
                <Link key={g.id} to={`/guides/${g.slug}`} className="p-3.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl block text-xs space-y-1">
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              ...
              <Link to="/wiki" className="text-xs text-purple-400 hover:underline">更多 →</Link>
            </div>
            <div className="space-y-3">
              {wikis.map(w => (
                <Link key={w.id} to={`/wiki/${w.slug}`} className="p-3.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl block text-xs space-y-1">
